...
export default testSuite(({ describe }) => {
	describe('config', async ({ test, describe }) => {
		...
		const openAiToken = 'KIMI_KEY=sk-abc';
		...
		test('set invalid KIMI_KEY', async () => {
			const { stderr } = await aicommits(['config', 'set', 'KIMI_KEY=abc'], {
				reject: false,
			});

			expect(stderr).toMatch(
				'Invalid config property KIMI_KEY: Must start with "sk-"'
			);
		});
		...
		await test('get config file', async () => {
			const { stdout } = await aicommits(['config', 'get', 'KIMI_KEY']);
