...
Testing requires passing in `KIMI_KEY` as an environment variable:

```sh
KIMI_KEY=<your KimiAI key> pnpm test
```


You can still run tests that don't require `KIMI_KEY` but will not test the main functionality:
```
pnpm test
```
